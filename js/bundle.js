...
var elementorFrontendConfig = {
  ...
  urls: {
    assets:
      "https://www.openskyinformation.com/wp-content/plugins/elementor/assets/"
  },
  settings: {
    page: [],
    general: {
  ...
  post: { id: 873, title: "Inicio", excerpt: "" }
};
...
function closeAllAccordions() {
  ...
  if (titles.length > 0) {
    titles.forEach(title => {
      ...
      const contentId = titleId.replace("title", "content");
      const contentContainer = document.querySelector("#" + contentId);

      title.classList.remove("elementor-active");

      contentContainer.classList.remove("elementor-active");
  ...
  if (titles.length > 0) {
    ...
    titles.forEach(title => {
      title.addEventListener("click", function (e) {
        closeAllAccordions();
        const titleId = e.target.getAttribute("id");
        const contentId = titleId.replace("title", "content");
        const contentContainer = document.querySelector("#" + contentId);
        if (!/elementor-active/.test(e.target.classList.value)) {
          e.target.classList.add("elementor-active");

          contentContainer.classList.add("elementor-active");
...
/**
 * method that transforms a form data to json data
 * @param {*} form 
 */
...
/**
 * method that validates the correct info to send the email
 */
  ...
  if ($customForm != null) {
    $customForm.addEventListener("submit", async function (e) {
      ...
      allInput.forEach(element => {
        ...
        const warning = element.parentNode.parentNode.parentNode.querySelector(
          "[data-warning]"
        );
        if (element.value.trim() != "") {
          ...
          warning.innerHTML = "El dato no se ha indicado";
        }
      });
      const json = serialize(e.target);
      try {
        let response = await sendRequestasync(json);
        swal("Hello world!");
...
/**
 * method that handle the send of form to the email
 * @param {*} data 
 */
async function sendRequestasync(data) {
  let headers = new Headers();
  let response;
  headers.append("Content-Type", "application/json");
  const url =
    "https://319b11tw1k.execute-api.us-east-1.amazonaws.com/dev/contact";
  try {
 ...
 * Last modified time: Dec 27 2019, 06:59:52
 */
